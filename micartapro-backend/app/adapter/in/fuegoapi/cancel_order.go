...
func init() {
	ioc.Register(cancelOrder)
}

func cancelOrder(
	s httpserver.Server,
	obs observability.Observability,
	cancelOrderUseCase order.CancelOrder,
) {
	fuego.Post(s.Manager, "/menu/{menuId}/orders/{aggregateId}/cancel",
		func(c fuego.ContextWithBody[events.OrderCancelledRequest]) (map[string]string, error) {
			spanCtx, span := obs.Tracer.Start(c.Context(), "cancelOrder")
			defer span.End()
			...
			err = cancelOrderUseCase(spanCtx, aggregateID, request)
			if err != nil {
				obs.Logger.ErrorContext(spanCtx, "error cancelling order", "error", err, "aggregateID", aggregateID)
				return nil, fuego.HTTPError{
			...
			obs.Logger.InfoContext(spanCtx, "order cancelled successfully", "aggregateID", aggregateID)
			return map[string]string{"status": "success"}, nil
		},
		option.Summary("cancelOrder"),
		option.Tags("orders"),
