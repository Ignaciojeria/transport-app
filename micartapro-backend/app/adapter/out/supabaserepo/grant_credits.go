...
func NewGrantCredits(supabase *supabase.Client, conf configuration.Conf) GrantCredits {
	return func(ctx context.Context, req billing.GrantCreditsRequest) (*billing.CreditTransaction, error) {
		...
		// Llamar a la función grant_credits usando HTTP POST directo
		rpcURL := fmt.Sprintf("%s/rest/v1/rpc/grant_credits", conf.SUPABASE_PROJECT_URL)
		...
		if err != nil {
			return nil, fmt.Errorf("error calling grant_credits RPC: %w", err)
		}
		...
		if err := json.Unmarshal(body, &results); err != nil {
			return nil, fmt.Errorf("error unmarshaling grant_credits result: %w", err)
		}

		if len(results) == 0 {
			return nil, fmt.Errorf("grant_credits function returned no results")
		}
