...
func NewSaveOrder(
	...
	LEFT JOIN node_infos d_ni ON d_ni.organization_country_id = org_country.organization_country_id AND d_ni.reference_id = ?;
		...
		return domain.Order{}, conn.Transaction(func(tx *gorm.DB) error {
			...
			saveIfNotExists := func(id int64, entity interface{}) error {
				if id == 0 {
			...
			if err := saveIfNotExists(result.CommerceID, &orderTable.Commerce); err != nil {
				return err
			}

			if err := saveIfNotExists(result.ConsumerID, &orderTable.Consumer); err != nil {
				return err
			}

			if err := saveIfNotExists(result.OriginContactID, &orderTable.OriginContact); err != nil {
				return err
			}
			...
			if result.DestinationContactID == 0 {
				if to.IsOriginAndDestinationContactEqual() {
					...
					orderTable.DestinationContact.ID = 0
			...
			if err := saveIfNotExists(result.OriginAddressID, &orderTable.OriginAddressInfo); err != nil {
				return err
			}
			...
			if result.DestinationAddressID == 0 {
				if to.IsOriginAndDestinationAddressEqual() {
					...
					orderTable.DestinationAddressInfo.ID = 0
			...
			if result.DestinationNodeInfoID == 0 {
				if to.IsOriginAndDestinationNodeEqual() {
					...
					orderTable.DestinationNodeInfo.ID = 0
