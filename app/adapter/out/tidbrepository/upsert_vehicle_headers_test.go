...
var _ = Describe("UpsertVehicleHeaders", func() {
	...
	BeforeEach(func() {
		...
		err := connection.DB.Exec("DELETE FROM vehicle_headers").Error
	...
	It("should insert headers if not exists", func() {
		...
		upsert := NewUpsertVehicleHeaders(connection)
		err := upsert(ctx, headers)
		Expect(err).ToNot(HaveOccurred())

		var dbHeaders table.VehicleHeaders
	...
	It("should allow same headers for different organizations", func() {
		...
		err = connection.DB.WithContext(context.Background()).
			Table("vehicle_headers").
			Where("commerce = ? AND consumer = ?", headers1.Commerce, headers1.Consumer).
			Count(&count).Error
		Expect(err).ToNot(HaveOccurred())
	...
	It("should fail if database has no vehicle_headers table", func() {
		ctx := createOrgContext(organization1)
		...
		Expect(err.Error()).To(ContainSubstring("vehicle_headers"))
	})
})
