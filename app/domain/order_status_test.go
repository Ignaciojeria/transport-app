...
var _ = Describe("OrderStatus", func() {
	Describe("DocID", func() {
		...
		It("should generate the same document ID for same status regardless of other fields", func() {
			orderStatus1 := OrderStatus{
				ID:        1,
				Status:    "completed",
				CreatedAt: time.Now(),
			...
			// Diferente ID y tiempo de creación pero mismo Status
			orderStatus2 := OrderStatus{
				ID:        2,
				Status:    "completed",
				CreatedAt: time.Now().Add(time.Hour), // 1 hora después
			}

			Expect(orderStatus1.DocID()).To(Equal(orderStatus2.DocID()))
		})
