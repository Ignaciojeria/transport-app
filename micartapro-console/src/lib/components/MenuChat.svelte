...
<div class="flex flex-col h-screen bg-white relative overflow-hidden">
  ...
  >
    <!-- Header del Preview -->
    <header class="border-b border-gray-200 bg-white px-4 py-2 flex items-center justify-between flex-shrink-0">
      <button
        onclick={() => {
          showPreview = false
          linkWasCopied = false
    ...
    <div class="flex-1 overflow-hidden iframe-container relative" style="padding-bottom: {menuUrl ? '80px' : '0'};">
      {#if menuUrl}
        {#key iframeKey}
          <iframe
            src={menuUrl}
    ...
    <!-- Botón flotante de compartir (barra fija en la parte inferior) -->
    {#if menuUrl}
      <div class="bg-white border-t border-gray-200 px-4 py-3 flex-shrink-0 safe-area-inset-bottom shadow-lg">
        <button
